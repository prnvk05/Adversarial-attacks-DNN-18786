...
def imshow(img, acc):
    ...
    # plt.show()
    plt.savefig(f'/root/Adversarial-attacks-DNN-18786/pics/car_og_{acc}.png')


# im_og = imshow(torchvision.utils.make_grid(torch.from_numpy(adv_examples[0][3][4])), 'loda')
im1 = adv_examples[0][3][3]
im2= adv_examples[1][6][3]
im3= adv_examples[2][6][3]
im4= adv_examples[3][6][3]
im5= adv_examples[4][6][3]
...
with open('/root/Adversarial-attacks-DNN-18786/saved_model/cutout_examples.pkl', 'rb') as f:
    cut_examples = pickle.load(f)
inv_normalize = transforms.Normalize(
    mean=[-0.485/0.229, -0.456/0.224, -0.406/0.255],
    std=[1/0.229, 1/0.224, 1/0.255]
)
