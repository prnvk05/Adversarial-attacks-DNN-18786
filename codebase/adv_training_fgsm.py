...
train_loader = torch.utils.data.DataLoader(
    datasets.CIFAR10(root='./data',download=True, train=True, transform=transforms.Compose([
        transforms.ToTensor(),
        normalize,
    ])),
    batch_size=1, shuffle=False,
...
def test( model, device, test_loader, epsilon ):
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        if len(adv_examples[target.item()]) <= 3000:
            adv_examples[target.item()].append((target.item(), adv_ex))
        
        tc = 0
        for buck in adv_examples:
            tc += len(buck)
        if tc >= 30000:
            break
...
for eps in epsilons:
    print("running:",eps)
    ex = test(model, device, train_loader, eps)
    examples.append(ex)
print("Done FGSM!")
...
# %%
labels = np.array([examples[]])
# labels = np.array([example[0] for example in cls_ in ex])
# labels = np.array([ex[0][0] for ex in examples[0]])
# imgs = np.array([pre_process(ex[0][1]) for ex in examples[0]])
